  ...
  <div class="add-form">
    ...
    <el-form ref="formRef"
      ...
      <el-form-item label="攻击手段"
                    prop="attackMethod">
        <el-select v-model="formData.attackMethod"
                   placeholder="请选择攻击手段"
                   @change="handleAttackMethodChange">
          <el-option v-for="item in attackMethodOptions"
                     :key="item.label"
                     :label="item.label"
                     :value="item.value" />
        </el-select>
      ...
      <template v-if="formData.damageType===2">
        <el-form-item label="武器型号"
                      prop="attackMethod">
          <el-select v-model="formData.weaponModel"
                     placeholder="请输入武器型号">
            <el-option v-for="item in formData.weaponModelList"
                       :key="item.id"
                       :label="item.name"
                       :value="item.id" />
          </el-select>
      ...
      <el-form-item label="是否有防御模型"
                    prop="defenseFlag">
        <el-switch v-model="formData.defenseFlag"
                   :active-value="1"
                   :inactive-value="0" />
      </el-form-item>
      <el-form-item label="防御武器"
                    prop="defensiveWeapon"
                    v-if="formData.defenseFlag===1">
        <el-select v-model="formData.defensiveWeapon"
                   placeholder="请选择防御武器">
          <el-option v-for="item in defensiveWeaponOptions"
                     :key="item.label"
                     :label="item.label"
                     :value="item.value" />
        </el-select>
      </el-form-item>
      <template v-if="formData.attackMethod==4&&formData.defenseFlag===1">
        <el-form-item label="风幕类型"
                      prop="windType">
          <el-input v-model="formData.windType"
                    placeholder="请输入风幕类型" />
        </el-form-item>
        <el-form-item label="风幕数量"
                      prop="windNum">
          <el-input v-model="formData.windNum"
                    placeholder="请输入风幕数量" />
        </el-form-item>
        <el-form-item label="风幕高度"
                      prop="windH">
          <el-input v-model="formData.windH"
                    placeholder="请输入风幕高度" />
        </el-form-item>
      </template>
	...
	setup(props, { emit }) {
		...
		const state = reactive<PlanMgrEditState>({
			...
			formData: {
				...
				createdAt: undefined,
				updatedAt: undefined,
				weaponModelList: [], // 添加 weaponModelList 属性
			},
			// 表单校验
			rules: {
				name: [{ required: true, message: '预案名称不能为空', trigger: 'blur' }],
				attackMethod: [{ required: true, message: '攻击手段不能为空', trigger: 'blur' }],
				weaponModel: [{ required: true, message: '武器型号不能为空', trigger: 'blur' }],
				defensiveWeapon: [{ required: true, message: '防御武器不能为空', trigger: 'blur' }],
			},
		});
		// 打开弹窗
